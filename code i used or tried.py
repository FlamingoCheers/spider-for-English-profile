...
for index in range(len(gwblockAllSetIn)):
    if gwblockAllSetIn[index]== None:
        print ('index')
    else:
        pass

newgwblock = list()
gwblocksetplacebo1 = list()
gwblocksetplacebo = list.copy(gwblockAllSetIn)
for index in range(len(gwblockAllSetIn)):
    gwblocksetplacebo.remove(gwblockAllSetIn[index])
    if gwblockAllSetIn[index] in gwblocksetplacebo:
        pass
    else:
        list.append(newgwblock,gwblockAllSetIn)
...
usageH = list()
for index in range(len(newgwblock)):
	h3Ele = newgwblock[index]
	h3EleH = h3Ele.find('h3')
	list.append(usageH,h3EleH)
	print ('usage'+str(index))
********************
...
defs = list()		
for index in range(len(newgwblock)):
	defEle = newgwblock[index].find('span',{'class':'def'})
	defText = defEle.text
...
for index in range(len(newgwblock)):
	examp_blocksVa = newgwblock[index]
...
lcNN = list()
for index in range(len(newgwblock)):
	learnerCEle = newgwblock[index].find('blockquote',{'class':'clc'})
	if learnerCEle == None :
		#print(index)
		lcEplacebo = soup('<blockquote class="clc" query="yes" resource="uk" show_less="yes"><div class="clc_before"><img alt="Cambridge Learner Corpus" src="/external/images/clc.png"/> Learner example: </div>Sorry, nothing is found here. Make up your own example maybe. <div class="src"> Best wishes</div></blockquote>',"html5lib")
...
for index in range(len(usagelist)):
	list.append(valueFor,  str('Usage: '+usagelist[index]+'^p'+'Definition: '+defslist[index]+'^p'+dictexamplist[index]+'^p'+learnerexamplist[index]+'^p'+'^p'))
pkF_valueFor = open(r'C:\myfirstgrab\c2tableFor3000.txt','wb')
